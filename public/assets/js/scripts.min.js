...
$(document).ready(function(){
    ...
    // Аккордионы
    function _accClearTransition(b){
        if(b._accTransitionEnd){
            b.removeEventListener("transitionend", b._accTransitionEnd);
            b._accTransitionEnd = null;
        }
        if(b._accTransitionTimer){
            clearTimeout(b._accTransitionTimer);
            b._accTransitionTimer = null;
        }
    }
    function _accFinishOpen(acc, b){
        _accClearTransition(b);
        if(acc.classList.contains("_active")) b.style.height = "auto";
    }
    function _accClose(acc){
        var b = acc.querySelector(".accordion__body"); if(!b) return;
        _accClearTransition(b);
        // Start from the actually rendered height. This also handles an open
        // body whose height has already switched to auto after the animation.
    ...
    function _accOpen(acc){
        var b = acc.querySelector(".accordion__body"); if(!b) return;
        _accClearTransition(b);
        acc.classList.add("_active");

        // Measure the complete answer after the active styles are applied.
        // Returning to auto when the transition finishes prevents clipping if
        // fonts, rich text or a viewport resize change the content afterwards.
        b.style.height = "auto";
        var targetHeight = b.scrollHeight;
        b.style.height = "0px";
        void b.offsetHeight;

        b._accTransitionEnd = function(e){
            if(e.target !== b || e.propertyName !== "height") return;
            _accFinishOpen(acc, b);
        };
        b.addEventListener("transitionend", b._accTransitionEnd);
        // transitionend is not emitted with reduced motion and can be skipped
        // when the browser tab loses focus, so always keep a safe fallback.
        b._accTransitionTimer = setTimeout(function(){
            _accFinishOpen(acc, b);
        }, 500);

        requestAnimationFrame(function(){
            if(acc.classList.contains("_active")) b.style.height = targetHeight + "px";
        });
    }
    // FAQ accordion — smooth height transition (was jQuery slideToggle → laggy).
    $(document).on("click", ".accordion__header", function(){
        var acc = $(this).closest(".accordion")[0]; if(!acc) return;
        var willOpen = !acc.classList.contains("_active");
        var sibs = acc.parentElement ? acc.parentElement.querySelectorAll(".accordion._active") : [];
        [].forEach.call(sibs, function(s){ if(s !== acc) _accClose(s); });
        if(willOpen) _accOpen(acc); else _accClose(acc);
    })
    // End Аккордионы

    // Попап
    $(".popup").each(function () {
        if (this.id === 'cheat-video') return;
    ...
    $(document).on("click", ".popup__video-wrap", function(e){
        e.stopPropagation();
    })
    ...
    $(document).on("click", "[data-popup]", function (e) {
        ...
        if (popup == 'auth' || popup == 'register' || popup == 'resetPass'){
            refreshCaptcha(popup);
        }
        openPopup(popup);
    })
    // End Попап
    ...
    function withIosCssMode(config) {
        if (!isIOS || config.iosCssMode === false) return config;
    ...
    /* v4.39.3 — desktop full-bleed rails: первый слайд стартует от колонки
       контента (под заголовком секции), сама лента тянется до краёв экрана.
       CSS-padding на контейнере ломал snap-математику Swiper (стрелку "назад"
       приходилось жать дважды), поэтому смещение задаём через нативный
       slidesOffsetBefore — Swiper учитывает его в translate и навигации.
       rect.left отдаёт визуальные px — делим на zoom body, т.к. Swiper
       считает в layout px. */
    function railOffsetBefore(el){
        if (!window.matchMedia("(min-width: 1024px)").matches) return 0;
        /* Recommendations use a real wrapper start gutter below. Swiper's
           slidesOffsetBefore changes its internal grid, but leaves the first
           card on the viewport edge and creates an extra first arrow step. */
        if (el.closest(".game-cheats__slider-container")) return 0;
        var content = el.closest(".content");
        if (!content) return 0;
        var zoom = parseFloat(window.getComputedStyle(document.body).zoom) || 1;
        return Math.max(0, Math.round(content.getBoundingClientRect().left / zoom));
    }
    function railOffsetAfter(el){
        if (!window.matchMedia("(min-width: 1024px)").matches) return 0;
        if (!el || !el.closest(".game-cheats__slider-container")) return 24;
        var firstSlide = el.querySelector(".swiper-slide");
        if (!firstSlide) return 24;
        var zoom = parseFloat(window.getComputedStyle(document.body).zoom) || 1;
        return Math.max(24,
            el.clientWidth
            - firstSlide.getBoundingClientRect().width / zoom
            + el.getBoundingClientRect().left / zoom
        );
    }
    function bindRailOffsets(swiper, el){
        var t;
        $(window).on("resize orientationchange", function(){
            window.clearTimeout(t);
            t = window.setTimeout(function(){
                syncRecommendationGutter(el);
                swiper.params.slidesOffsetBefore = railOffsetBefore(el);
                swiper.params.slidesOffsetAfter = railOffsetAfter(el);
                swiper.update();
            }, 140);
        });
    }
    function syncRecommendationGutter(el){
        var section = el.closest(".game-cheats__slider-container");
        var heading = section && section.querySelector(".section-caption");
        if (!heading) return;
        var zoom = parseFloat(window.getComputedStyle(document.body).zoom) || 1;
        var gutter = Math.max(0,
            (heading.getBoundingClientRect().left - el.getBoundingClientRect().left) / zoom
        );
        el.style.setProperty("--recommendation-start-gutter", gutter + "px");
    }

    $(".game-cheats-slider").each(function(){
        var $slider = $(this);
        var $container = $slider.closest(".game-list, .game-cheats__slider-container");

        /* Phone and iPad rails are native scroll containers.  Running a
           transform-based Swiper on top of them created two independent
           positions: touch moved scrollLeft while the arrows moved
           translate3d.  After several taps the card could therefore stop
           half outside the viewport. */
        if (document.documentElement.classList.contains("game-rails-native")) {
            this.classList.add("game-rail--native");
            return;
        }

        var isRecommendationRail = !!$slider[0].closest(".game-cheats__slider-container");
        if (isRecommendationRail) syncRecommendationGutter(this);
        var railSwiper = new Swiper($slider[0], withIosCssMode({
            /* Let the existing 290px card width own layout before and after
               Swiper starts. Numeric slide counts rewrote every card from
               290px to ~343px after first paint and caused a visible jump. */
            slidesPerView: "auto",
            slidesOffsetBefore: railOffsetBefore(this),
            slidesOffsetAfter: railOffsetAfter(this),
            spaceBetween: 22,
            touchEventsTarget: 'container',
            ...
            navigation: {
                prevEl: $container.find(".slider-arrows__prev")[0],
                nextEl: $container.find(".slider-arrows__next")[0]
            },
            /* Game rails use Swiper's transform mode on every touch device.
               Mixing native cssMode scrolling with transformed slide widths
               made rapid arrow presses accumulate into an off-grid position. */
            iosCssMode: false,
            cssMode: false,
            speed: 360,
            ...
            breakpoints: {
                ...
                /* Thresholds lowered ~12% so the higher card counts kick in
                   earlier — body{zoom:0.9} on tablet/desktop shrinks the
                   effective width, otherwise iPad landscape got only 2 huge
                   ("вытянутые") cards. */
                760: {
                    /* Tablet cards have an explicit responsive width in CSS.
                       Keeping this auto prevents Swiper from overwriting that
                       width after an iPad orientation change. */
                    slidesPerView: "auto",
                    spaceBetween: 16
                },
                1120: {
        ...
        /* This Swiper build does not expose the instance on element.swiper.
           Keep an explicit reference for the shared arrow/orientation logic. */
        $slider[0].__gameRailSwiper = railSwiper;
        bindRailOffsets(railSwiper, this);
    })
    ...
    /* A menu opened in portrait used to retain its mobile _active state after
       rotating an iPad to the 1180–1400px desktop header. That invisible
       mobile layer then sat over the desktop navigation. Reset it after the
       viewport has settled at the real desktop breakpoint. */
    var headerViewportTimer;
    $(window).on("resize orientationchange", function(){
        window.clearTimeout(headerViewportTimer);
        headerViewportTimer = window.setTimeout(function(){
            if (window.matchMedia("(min-width: 1180px)").matches && $(".header__menu").hasClass("_active")) {
                setMobileMenu(false);
            }
        }, 140);
    })
    ...
    ;(()=>{
        ...
        if (false) new Swiper('.cheat-functions .swiper', withIosCssMode({
            slidesPerView: "auto",
            spaceBetween: 18,
            touchEventsTarget: 'container',
            passiveListeners: false,
    ...
    ;(()=>{
        ...
        let cardsEl = document.querySelector(".game-cards-slider");
        if (!cardsEl) return;
        let cardsSwiper = new Swiper(cardsEl, withIosCssMode({
            slidesOffsetBefore: railOffsetBefore(cardsEl),
            slidesOffsetAfter: railOffsetAfter(),
            spaceBetween: 20,
            touchEventsTarget: 'container',
            passiveListeners: false,
            touchReleaseOnEdges: false,
        ...
        bindRailOffsets(cardsSwiper, cardsEl);
    })();
    ...
    $(document).on("click", ".profile__settings-block_input .profile__settings-block__edit-btn", function(){
        ...
        if (input.attr("readonly")){
            ...
            $('.'+input.attr("id")+' .profile__settings-block__edit-btn').attr('onclick', 'btnSave("'+input.attr("id")+'")');
        } else {
            input.attr("readonly", true);
            $('.'+input.attr("id")+' .profile__settings-block__edit-btn').removeAttr("onclick");
    ...
    // ТЗ §9 — the player kind and embed src are computed server-side
    // (resources/views/user/cheat.blade.php), so this only builds the element.
    $("#cheat-video").on("popupOpen", function(){
        var container = $("#video-player-container");
        if (container.length === 0) return;
        var kind = container.data("video-kind");
        var src  = container.data("embed-src");
        var url  = container.data("video-url");
        container.empty().append('<span class="video-modal__spinner" aria-hidden="true"></span>');
        container.removeClass("is-ready");

        function ready(){ container.addClass("is-ready"); container.find(".video-modal__spinner").remove(); }
        // Never leave the frame hidden behind a spinner if the embed is slow,
        // blocked or never fires load.
    ...
    $("#cheat-video").on("popupClose", function(){
        var vid = $("#video-player-container video")[0];
        if (vid) { vid.pause(); vid.removeAttribute('src'); vid.load(); }
        // Blank the iframe first — otherwise YouTube audio can outlive the
        // DOM removal for a moment.
        var f = $("#video-player-container iframe")[0];
        if (f) { f.src = "about:blank"; }
        $("#video-player-container").empty().removeClass("is-ready");
    })

    // Escape closes whichever popup is open (there was no key handling at all).
    $(document).on("keydown", function(e){
        if (e.key !== "Escape" && e.keyCode !== 27) return;
        if (!$(".popup._active").length) return;
        closePopup();
    })
})
...
function showNotification(text,type){
    let offsetBottom = $(window).height()/200;
    if ($(window).width() < 992){
        offsetBottom += 74;
    }
    if ($(".notification-container").length == 0) $("body").append(`<div class="notification-container" style="z-index:998"></div>`)
    ...
    if($(".notification").length > 0){
        $(".notification").each(function(){
            $(this).css("bottom",parseInt($(this)[0].style.bottom) + ($(this).next().innerHeight() || $(this).height()/3)+ 10);
            if(parseInt($(this)[0].style.bottom,10) > window.innerHeight/2){
                $(this).fadeOut(300);
            }
        })
    }
...
function freeze() {
    if ($("html").css("position") != "fixed") {
        ...
        /* Preserve the document's exact content width while the scrollbar is
           removed. Without this compensation centered grids/cards expand by
           the scrollbar width and the whole product page visibly jerks. */
        var scrollbarGap = Math.max(0, window.innerWidth - document.documentElement.clientWidth);
        $("html").css("overflow-y", "hidden");
        /* Locking <html> offsets the entire document by -top. The header is
           absolute in normal layout, so mark the lock state for CSS to pin it
           back to the viewport while a modal is open. */
        ...
        $("html").css({
            ...
            "position": "fixed",
            "top": -top,
            "padding-right": scrollbarGap ? scrollbarGap + "px" : ""
        });
    }
}
...
function unfreeze() {
    if ($("html").css("position") == "fixed") {
        ...
        /* Let sticky-header update only after the original scroll position
           and regular document flow have both been restored. */
...
function initCustomSelect(selectElement){
    const self = selectElement,
        select = selectElement.find("select"),
        isSearch = selectElement.find(".select__search").length != 0;
    ...
    if (self.hasClass("_unfold")){
        self.append(`
                <div class="select__selected"></div>
            `);
        if (self.find(".select__inner").length == 0){
            self.append(`<div class="select__inner"></div>`)
        }
        inner = self.find(".select__inner");
    ...
    if (!inner) inner = self.find(".select__inner");
    ...
    inner.find(".select__option").click(function(){
        ...
        if (select[0]) select[0].dispatchEvent(new Event("change", { bubbles: true }));
        if (isSearch){
    ...
    if (exists(self.find("option[disabled]"))){
        self.find(".select__selected").html(self.find("option[disabled]").text());
    } else{
        let selectedOption = self.find(`.select__option[data-value="${self.find("option[selected]").val()}"]`);
        if (exists(selectedOption)){
            self.find(".select__selected").html(selectedOption.html());
            self.find(`.select__option[data-value="${self.find("option[selected]").val()}"]`).addClass("_active");
        } else{
            selectedOption = self.find(".select__option").eq(0);
            self.find(".select__selected").html(selectedOption.html());
...
function refreshCaptcha(id) {
    ...
    hcaptcha.render("#"+id+" .h-captcha", {
        sitekey: '54b4a874-7e58-4e34-ba84-a88ebfd4188c',
...
// option = {value: string, displayedValue: string}
function customSelectAddOption(selectElement, option){
    if (!selectElement.customSelect) return;

    const {self, select, inner, isSearch} = selectElement.customSelect;

    select.append(`<option value="${option.value}">${option.value}</option>`)

    const newOption = $(`<div class="select__option" data-value="${option.value}">${option.displayedValue}</div>`);
    newOption.appendTo(inner);
...
var headerTimeline, loginTimeline,
    initAnimations = () => {};

$(document).ready(function() {
    $("body").addClass("_loaded")
    if (mainHasClass("main") && headerTimeline){
        headerTimeline.resume();
    }
    if (loginTimeline){
        loginTimeline.resume();
    }
    setTimeout(function(){
...
// Header intro plays ONCE per session. On later in-session navigations the
// header stays static (no opacity:0 start), so it never jerks/flashes during
// page transitions (and the View Transitions snapshot of it stays stable).
var _hdrFirstVisit = true;
try { _hdrFirstVisit = !sessionStorage.getItem("fnrus_hdr_seen"); sessionStorage.setItem("fnrus_hdr_seen", "1"); } catch (e) { _hdrFirstVisit = true; }

if (_hdrFirstVisit && mainHasClass("main")){
    ...
    if (ww >= 1440){
        ...
        .from(".header__hamburger", {x: 50, opacity: 0, duration: .3, ease: $ease, clearProps: "transform,opacity"}, '-=.25')
    } else{
        headerTimeline
        .from(".header__lang, .header__search-btn, .header__hamburger", {y: -20, opacity: 0, duration: .3, ease: $ease, stagger: .1, clearProps: "transform,opacity"}, '-=.5')
        .from(".header__login", {x: 20, opacity: 0, duration: .3, ease: $ease, clearProps: "transform,opacity"}, '-=.25')
    }
...
if (_hdrFirstVisit && !mainHasClass("main")){
    ...
    .from(".header__login", {x: ww >= 768 ? 50 : 20, opacity: 0, duration: ww >= 768 ? .5 : .3, ease: $ease, delay: .25, clearProps: "transform,opacity"});
    loginTimeline.pause();
}

/* Анимации витрины перенесены в animations.js (IntersectionObserver). */
initAnimations = () => {};
